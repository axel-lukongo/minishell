...
int	wildcard_between(char **files, char **res, t_alloc **alloc, char *str,
	int is_after)
{
	int		i;
	int		n;
	...
	to_match_end = ft_substr(str, when_is_char(str, '*') + 1, ft_strlen(str),
			*alloc);
	while (files[i])
	{
...
int	wildcard_start(char **files, char **res, t_alloc **alloc, char *str,
	int is_after)
{
	char	*to_match_end;
	int		i;
	...
	n = 0;
	i = 0;
	to_match_end = ft_substr(str, when_is_char(str, '*') + 1,
			ft_strlen(str),*alloc);
	while (files[i])
	{
...
int	wildcard_end(char **files, char **res, t_alloc **alloc, char *str,
	int is_after)
{
	char	*to_match_start;
	int		i;
	...
	while (strs[i])
	{
		if (is_char(strs[i], '*') > 0)
		{
			...
			if (strs[i][0] == '*' && strs[i][1] == 0)
			{
				...
				{
					if (strs[i + 1] != NULL)
						res = ft_strjoin(res, ft_strjoin("*", " ", &alloc),
								&alloc);
					else
						res = ft_strjoin(res, "*", &alloc);
				}
					...
					!= '*')
			{
				if (!(wildcard_between(files, &res, &alloc, strs[i],
							strs[i + 1] != NULL)))
				{
					if (strs[i + 1] != NULL)
						res = ft_strjoin(res, ft_strjoin(strs[i], " ", &alloc),
								&alloc);
					else
				...
				!= '*')
			{
				if (!(wildcard_start(files, &res, &alloc, strs[i], strs[i + 1]
							!= NULL)))
				{
					if (strs[i + 1] != NULL)
						res = ft_strjoin(res, ft_strjoin(strs[i], " ", &alloc),
			...
			{
				if (!(wildcard_end(files, &res, &alloc, strs[i], strs[i + 1]
							!= NULL)))
				{
					if (strs[i + 1] != NULL)
						res = ft_strjoin(res, ft_strjoin(strs[i], " ", &alloc),
